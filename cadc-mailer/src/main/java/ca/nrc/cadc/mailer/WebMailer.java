...
package ca.nrc.cadc.mailer;

import ca.nrc.cadc.mailer.Mailer;
...
{
    private static final String ORG_FROM_EMAIL_ADDRESS_KEY_PREFIX =
            "ORG_FROM_EMAIL_ADDRESS_";
    ...
    private static final String ORG_EMAIL_SENT_PAGE_KEY_PREFIX =
            "ORG_EMAIL_SENT_PAGE_";
    ...
    public WebMailer(final SiteRole siteRole, final String hostName,
    ...
    //needs to be abstract. Password and invite can use this separately. Invite will link to the group management page and this to the password reset page.

    URL createLink(final String token) throws IOException
    {
        final String linkText = getURLPrefix() +
                                i18nBundle.getString(NEW_PASSWORD_PAGE_KEY_PREFIX
                                                     + siteRole.name()) +
    ...
    private String assembleMessage(final String token) throws IOException
    {
        final String organizationAcronym =
                i18nBundle.getString(ORG_ACRONYM_KEY_PREFIX + siteRole.name());
        final String organizationName =
                i18nBundle.getString(ORG_NAME_KEY_PREFIX + siteRole.name());
        final String linkText = createLink(token).toExternalForm();
        return String.format(i18nBundle.getString(EMAIL_BODY_CONTENT_KEY),
                             organizationAcronym, linkText, linkText,
                             organizationName);
    }
     ...
     * @throws IOException
     */
    public String sendMail(final String mailAddress, final String token)
            throws IOException, MessagingException
    {
        ...
        return getURLPrefix()
               + i18nBundle.getString(ORG_EMAIL_SENT_PAGE_KEY_PREFIX
                                      + siteRole.name());
    }

   Mailer getMailer(final String mailAddress, final String message)
    {
        final Mailer mailer = createMailer();

        mailer.setFrom(i18nBundle.getString(ORG_FROM_EMAIL_ADDRESS_KEY_PREFIX
                                            + siteRole.name()));
